...
\begin{figure}[ht]
  ...
  \caption{The geometric representation of CCA, on a toy subset of the WELL-China
    ...
    all of $\reals^{3}$. The ellipses are unit balls with respect to each data
    set's covariance matrix, estimated using all samples (not just the three
    displayed). Two arrows are the first CCA directions associated with the two
    measurement types. Evidently, the correlation between these measurement
    types, restricted to these few samples and features, is quite high, since
    the arrows point in nearly the same direction.
...
\begin{figure}[ht]
  ...
  \caption{The scores resulting from CCA applied to the combined body
    composition and microbial abundance data set.\label{fig:cca_scores_weight} }
\end{figure}
