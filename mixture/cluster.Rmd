...
stan_data <- list(
  ...
  "alpha" = rep(10, 4),
  "gamma" = rep(0.01, filter_thresh)
)

m <- stan_model("dirichlet_multinomial.stan")
dm_fit <- vb(m, stan_data)
...
p_hat <- apply(dm_res$p, c(2, 3), mean)
p_df <- p_hat %>%
  melt(
    varnames = c("k", "v"),
    value.name = "p"
...
assignments, rather than having to guess them from the fitted clusters.

```{r lda_fit}
m <- stan_model("lda_counts.stan")
lda_fit <- vb(m, stan_data)
lda_res <- extract(vb_fit)
```
